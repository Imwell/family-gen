...
<script>
    ...
    layui.use('table', function(){
        ...
        table.render({
            elem: '#LAY_table_role'
            ,url: '${pageContext.request.contextPath}/system/role/findListPage?auth_type=${pd.auth_type}&auth_organize_ids=${pd.auth_organize_ids}'
                ,cols: [[
        ...
        $('#dels').on('click',function (){
            ...
            if(data.length>0){
                layer.confirm('真的要删除数据么', function(index){
                    ...
                    $.get("${pageContext.request.contextPath}/system/role/del?ids="+ids,null,function(res){
                        if (res.success) {
        ...
        table.on('tool(role)', function(obj){
            ...
            if(obj.event === 'detail'){
                ...
                layer.confirm('真的要删除数据么', function(index){
                    $.get("${pageContext.request.contextPath}/system/role/del?ids="+data.id,null,function(res){
                        if (res.success) {
                            layer.msg("数据删除成功。", {time: 2000});
                            location.replace(location.href);
                        }else{
    ...
    function edit(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/system/role/toUpdate?id='+id,
            area: ['800px', '500px'],
            end: function () {
                location.reload();
            }
    ...
    function auth(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/system/role/toAuth?role_id='+id,
            area: ['320px', '500px'],
            end: function () {
                //location.reload();
            }
