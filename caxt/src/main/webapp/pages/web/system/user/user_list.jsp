...
//            ,defaultToolbar: ['filter', 'print', 'exports']
            ,url: '${pageContext.request.contextPath}/system/user/findListPage?auth_type=${pd.auth_type}&auth_organize_ids=${pd.auth_organize_ids}'
                ,cols: [[
...
//                ,{field:'create_time', title: '创建时间'}
        ...
        form.on('switch(statusCheck)', function(obj){
            ...
            $.get("${pageContext.request.contextPath}/system/user/updateStatus?id="+this.value+"&status="+status,null,function(res){
                console.log(res);
                if (res.success) {
                    layer.tips("状态修改成功。");
                }else{
        ...
        $('#dels').on('click',function (){
            ...
            if(data.length>0){
                layer.confirm('删除数据会同步删除【用户组织信息、用户权限信息】，真的要删除数据么?', function(index){
                    ...
                    $.get("${pageContext.request.contextPath}/system/user/del?ids="+ids,null,function(res){
                        if (res.success) {
        ...
        $('#synchUserWxAccount').on('click',function (){
            ...
            $.get("${pageContext.request.contextPath}/system/user/synchUserWxAccount?",null,function(res){
                layer.close(index);
                if (res.success) {
                    console.log(res.msg);
                    layer.alert(res.msg);
//                    location.replace(location.href);
    ...
    function auth(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/system/user/toAuth?user_id='+id,
            area: ['800px', '500px'],
            end: function () {
                //location.reload();
            }
    ...
    function toImport(){
        ...
        if(organize_id==''||organize_id==null){
            ...
            parent.layer.open({
                ...
                maxmin: true,
                type: 2,
                content: '${pageContext.request.contextPath}/system/user/toImport?organize_id='+organize_id+'&organize_name='+organize_name,
                area: ['500px', '300px'],
                end: function () {
                    location.reload();
                }
    ...
    function downloadExcel(){
        window.location.href="${pageContext.request.contextPath}/system/user/downloadExcel";
    }
