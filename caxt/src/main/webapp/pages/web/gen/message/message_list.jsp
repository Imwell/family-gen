...
<script>
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,url: '${pageContext.request.contextPath}/gen/message/findListPage?type=${pd.type}&auth_type=${pd.auth_type}&auth_organize_ids=${pd.auth_organize_ids}'
                ,cols: [[
        ...
        $('#dels').on('click',function (){
            ...
            if(data.length>0){
                layer.confirm('真的要删除数据么', function(index){
                    ...
                    $.get("${pageContext.request.contextPath}/gen/message/del?ids="+ids,null,function(res){
                        if (res.success) {
        ...
        table.on('tool(message)', function(obj){
            ...
            if(obj.event === 'detail'){
                ...
                layer.confirm('真的要删除数据么', function(index){
                    $.get("${pageContext.request.contextPath}/gen/message/del?ids="+data.id,null,function(res){
                        if (res.success) {
                            layer.msg("数据删除成功。", {time: 2000});
                            location.replace(location.href);
                        }else{
    ...
    function add(){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gen/message/toAdd?type=${pd.type}',
            area: ['800px', '500px'],
            end: function () {
                location.reload();
            }
    ...
    function edit(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gen/message/toUpdate?id='+id,
            area: ['800px', '500px'],
            end: function () {
                location.reload();
            }
    ...
    function info(id){
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gen/message/findInfo?id='+id,
            area: ['800px', '500px']
        });
    }
    ...
    function updateStatus(id,status){
        $.get("${pageContext.request.contextPath}/gen/message/updateStatus?id="+id+"&status="+status,null,function(res){
