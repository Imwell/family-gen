...
<form class="layui-form" action="${pageContext.request.contextPath}/gen/member/update" id="form">
    <table width="90%">
        ...
        <tr>
            ...
            <td colspan="3"><input type="checkbox" name="open_wife" lay-filter="openButton" <c:if test="${p.open_wife == 'on'}"> checked </c:if>  title="开启【添加丈夫(妻子)】功能"></td>
        </tr>
...
<script>
    ...
    layui.use(['form'], function(){
        ...
        if (${p.self} === 1) {
            $("input[name='self']").get(0).checked = true;
        } else {
            $("input[name='self']").get(1).checked = true;
        }
    ...
    function getGeneration(family_id, generation_id) {
        $.ajax({
            type: "GET",//方法类型
            dataType: "json",//预期服务器返回的数据类型
            url: "${pageContext.request.contextPath}/gen/member/findFatherList?family_id="+family_id+"&generation_id="+generation_id,
            data: "",
            success: function (res) {
                if (res.success) {
                    console.log(res)
    ...
    const member_id = '${p.id}';
    function getLevel(family_id, generation_id) {
        $.ajax({
            type: "GET",//方法类型
            dataType: "json",//预期服务器返回的数据类型
            url: "${pageContext.request.contextPath}/gen/member/findMemberLevelList?family_id="+family_id+"&generation_id="+generation_id+"&self_member_id="+member_id,
            data: "",
            success: function (res) {
                if (res.success) {
                    var tt = '<option value=""></option>';
