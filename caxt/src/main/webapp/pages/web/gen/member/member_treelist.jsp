...
<script>
    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,url: '${pageContext.request.contextPath}/gen/member/findListPage?auth_type=${pd.auth_type}&auth_organize_ids=${pd.auth_organize_ids}'
                ,cols: [[
...
//                        generation_id: generation_id
        ...
        $('#dels').on('click',function (){
            ...
            if(data.length>0){
                layer.confirm('真的要删除数据么', function(index){
                    ...
                    $.get("${pageContext.request.contextPath}/gen/member/del?ids="+ids,null,function(res){
                        if (res.success) {
        ...
        table.on('tool(member)', function(obj){
            ...
            if(obj.event === 'detail'){
                ...
                layer.confirm('真的要删除数据么', function(index){
                    $.get("${pageContext.request.contextPath}/gen/member/del?ids="+data.id,null,function(res){
                        if (res.success) {
                            layer.msg("数据删除成功。", {time: 2000});
                            location.replace(location.href);
                        }else{
...
//                var generation_id = $('#generation_id option:selected') .val();
    ...
    function add(){
        ...
        parent.layer.open({
            ...
            maxmin: true,
            type: 2,
            content: '${pageContext.request.contextPath}/gen/member/toAdd?family_id='+family_id+'&generation_id='+generation_id,
            area: ['800px', '500px'],
            end: function () {
                location.reload();
            }
