        ...
        if(path.matches(Const.NO_INTERCEPTOR_PATH)){
            ...
            if (user != null) {
                ...
                if (request.getHeader("x-requested-with") != null && request.getHeader("x-requested-with").equalsIgnoreCase("XMLHttpRequest")) {
                    ...
                    json.setUrl(request.getContextPath() + "/system/login/toLogin");
                    json.setLoseSession("loseSession");
                    json.setMsg("登录失效，正在跳转。。。");
                    response.setContentType("text/html;charset=utf-8");
                    ObjectMapper objMapper = new ObjectMapper();
                    JsonGenerator jsonGenerator = objMapper.getFactory()
                    ...
                    out.println("<html>");
                    out.println("<script>");
                    out.println("window.open ('"+request.getContextPath()+"/system/login/toLogin','_top')");
                    out.println("</script>");
                    out.println("</html>");
                    bol = false;
                }
