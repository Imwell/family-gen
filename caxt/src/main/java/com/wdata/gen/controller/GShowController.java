...
public class GShowController extends BaseController {
    ...
    public String index(ModelMap map, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if(Verify.verifyIsNotNull(pd.get("id"))){
            ...
            if (null == p) { // 没有前代，查询当前代及其子类
                PageData pageData = new PageData();
                pageData.put("id", myPd.get("id"));
                p = showService.findMemberInfo(pageData);
            }
    ...
    @RequestMapping(value = "/previewTree", method = RequestMethod.GET)
    public String previewIndex(ModelMap map, HttpServletRequest request) {
        PageData pd = new PageData(request);

        List<PageData> familyLs = familyService.findList(pd);
        map.put("familyLs",familyLs);
        map.put("pd",pd);
        return "web/gen/show/show_tree_preview";
    }

    @RequestMapping(value = "/getTree/{id}", method = RequestMethod.GET)
    public void getTree(@PathVariable String id, HttpServletRequest request, HttpServletResponse response) throws IOException {
        PageData pd = new PageData(request);
        //查询数据
        pd.put("family_id", id);
        //查询出根节点
        PageData p = showService.findRootMemberInfo(pd);
        if (null == p) {
            this.writeJson(response, null);
            return;
        }
        pd.put("father_id", p.get("id"));
        List<PageData> list = showService.findMemberList(pd);
     ...
     *  
     */ 
    @RequestMapping(value = "/findTreeAllJson/{id}", method = RequestMethod.GET)
    public void findTreeAllJson(@PathVariable int id, HttpServletRequest request, HttpServletResponse response) throws IOException {
        PageData pd = new PageData(request);
        //查询数据
        pd.put("family_id", id);
        //查询出根节点
